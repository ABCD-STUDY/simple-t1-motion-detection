...
int main(int argc, char **argv)
{
  ...
  if (t1.size() == 0)
  {
    std::cerr << "Error: Input directory \"" << argv[1] << "\" does not exist or no files found in input directory." << std::endl;
    exit(66); // cannot open input error
  }
  ...
  fprintf(stdout, "[");
  for (int volidx = 0; volidx < t1.size(); volidx++) {
    NormalizeFilterType::Pointer normalizeFilter = NormalizeFilterType::New();
    normalizeFilter->SetInput(t1[volidx]->v);
    typedef itk::StatisticsImageFilter<DICOMImageType> StatisticsFilterType;

    DICOMImageType::RegionType region = t1[volidx]->v->GetLargestPossibleRegion();
    DICOMImageType::SizeType regionSize = region.GetSize();
